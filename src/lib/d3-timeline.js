    ...
    function timeline (gParent) {
        ...
        g.each(function (d, i) {
          d.forEach(function (datum, index) {
            ...
            if (stacked && Object.keys(yAxisMapping).indexOf(index) == -1) {
              yAxisMapping[index] = (datum.yAxisMapping + 1) || maxStack++;
              if (datum.yAxisMapping + 1 > maxStack) maxStack = datum.yAxisMapping + 2;
            }
      ...
      if (width > gParentSize.width) {
        function move() {
          ...
          var x = Math.min(0, Math.max(-2200, d3.event.translate[0]));
          zoom.translate([x, 0]);
